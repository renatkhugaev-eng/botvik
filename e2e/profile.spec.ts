...
test.describe('Профиль пользователя', () => {
  ...
  test('URL должен быть /miniapp/profile', async ({ page }) => {
    await page.goto('/miniapp/profile');
    
    expect(page.url()).toContain('/miniapp/profile');
  });
  ...
  test('должна работать прокрутка', async ({ page }) => {
    ...
    // Скролл должен работать (или страница короткая)
    const scrollY = await page.evaluate(() => window.scrollY);
    expect(scrollY).toBeGreaterThanOrEqual(0);
  });

  test('не должно быть JavaScript ошибок при загрузке', async ({ page }) => {
    let jsError = false;
    ...
    // Допускаем некоторые ошибки в dev режиме
