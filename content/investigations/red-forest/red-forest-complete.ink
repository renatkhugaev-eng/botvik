// ═══════════════════════════════════════════════════════════════════════════════
// КРАСНЫЙ ЛЕС — Полная история (Эпизоды 1-5 + Расширение 6-15)
// ═══════════════════════════════════════════════════════════════════════════════
// 
...
// 
// РАСШИРЕНИЕ v3.0 (Январь 2026):
// - Hub-система для дней 6-15
// - Модульные локации и побочные квесты
// ═══════════════════════════════════════════════════════════════════════════════
...
INCLUDE new-content/extension-variables.ink
INCLUDE new-content/hub-system.ink
INCLUDE new-content/locations/market.ink
...
LIST CluesD = expedition_1890, serafim_legends, church_symbols, old_mine_secret
...
LIST CluesE = klava_testimony, fyodor_map, gromov_confession, vera_research, old_photos, zorin_warning_letters, zorin_night_photos, zorin_hidden_notes
...
// Время: история длится 15 дней (15-29 ноября)
// Дни 1-4: основной сюжет, дни 5-14: расширенное расследование, день 15: финал
VAR days_remaining = 15
VAR current_day = 1
...
VAR trust_klava = 30   // Клавдия — хозяйка ресторана, начальное доверие
...
LIST Inventory = item_flashlight, item_gun, item_notebook, item_camera, item_lockpick, item_vodka, item_medicine, item_cerebrolizin
...
// Полнолуние — 29 ноября 1986 года (через 14 дней после начала)
VAR moon_phase = 0  // 0=новолуние, 50=полумесяц, 100=полнолуние
VAR ritual_countdown = 14  // Дней до ритуала (финал на день 15)
...
// ═══════════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА РАССУДКА (v3.0)
// ═══════════════════════════════════════════════════════════════════════════════

// Типы видений: пророческие дают подсказки, ложные отвлекают
LIST VisionTypes = vtype_prophetic, vtype_false, vtype_memory, vtype_warning
VAR active_visions = ()

// Счётчики видений для трекинга
VAR prophetic_visions_seen = 0
VAR false_visions_seen = 0
VAR total_visions_seen = 0

// Скрытые пути, открытые низким рассудком
LIST SanityPaths = path_voices_cave, path_mirror_truth, path_dead_speak, path_see_through
VAR unlocked_sanity_paths = ()

// NPC реакции на состояние рассудка (уровни)
// 0 = не замечают, 1 = беспокоятся, 2 = боятся, 3 = отказываются говорить
VAR npc_sanity_awareness_gromov = 0
VAR npc_sanity_awareness_vera = 0
VAR npc_sanity_awareness_tanya = 0
VAR npc_sanity_awareness_serafim = 0
VAR npc_sanity_awareness_klava = 0

// Флаг: NPC видели безумие Сорокина
VAR witnessed_breakdown_public = false
VAR witnessed_breakdown_tanya = false
VAR witnessed_breakdown_vera = false

// ═══════════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ
// ═══════════════════════════════════════════════════════════════════════════════

// Эксклюзивные улики от NPC (требуют высокое доверие)
LIST ExclusiveClues = excl_gromov_key, excl_vera_medcards, excl_serafim_catacombs, excl_tanya_diary, excl_fyodor_safe_path, excl_chernov_letters
VAR obtained_exclusive_clues = ()

// Просьбы NPC (квесты для повышения доверия)
LIST NPCRequests = req_gromov_wife, req_gromov_daughter, req_vera_medicine, req_vera_patient, req_serafim_icon, req_serafim_prayer, req_tanya_lastday, req_tanya_photos
VAR completed_requests = ()
VAR active_request = 0  // 0=нет, 1=громов, 2=вера, 3=серафим, 4=таня

// Предательства и спасения
VAR betrayed_by_gromov = false
VAR betrayed_by_vera = false  
VAR saved_by_tanya = false
VAR saved_by_serafim = false
VAR saved_by_fyodor = false

// Конфликты интересов — кто знает о чьих секретах
VAR told_gromov_about_vera = false
VAR told_vera_about_cult = false
VAR told_tanya_about_danger = false
VAR told_serafim_about_chernov = false

// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНЫЕ РАССЛЕДОВАНИЯ
// ═══════════════════════════════════════════════════════════════════════════════

// Состояние побочных квестов
LIST SideQuests = sq_none, sq_letters_started, sq_letters_trusted, sq_letters_ignored, sq_letters_solved, sq_dogs_started, sq_dogs_solved, sq_photo_found, sq_photo_solved, sq_nurse_started, sq_nurse_helped, sq_nurse_betrayed
VAR active_sidequests = ()

// Анонимные письма
VAR letters_received = 0
VAR letters_followed = 0
VAR letter_author_known = false

// ═══════════════════════════════════════════════════════════════════════════════
// ИНТЕРЛЮДИИ
// ═══════════════════════════════════════════════════════════════════════════════

// Флаги пройденных интерлюдий
VAR interlude_nightmare_1_played = false
VAR interlude_nightmare_2_played = false
VAR interlude_nightmare_3_played = false
VAR interlude_zorin_flashback = false
VAR interlude_expedition_flashback = false

// Результаты кошмаров
VAR nightmares_won = 0
VAR nightmares_lost = 0

// ═══════════════════════════════════════════════════════════════════════════════
// НОВЫЕ ЛОКАЦИИ
// ═══════════════════════════════════════════════════════════════════════════════

LIST NewLocations = loc_zorin_apartment, loc_school, loc_cemetery, loc_mine_entrance, loc_backroom
VAR visited_new_locations = ()

// Квартира Зориных
VAR zorin_apt_unlocked = false
VAR zorin_apt_searched = false
VAR found_zorin_letters = false
VAR found_zorin_photos = false
...
// ═══════════════════════════════════════════════════════════════════════════════
// СИСТЕМА ВИДЕНИЙ (v3.0) — Галлюцинации как механика
// ═══════════════════════════════════════════════════════════════════════════════

// Функция: может ли игрок видеть видения
=== function can_see_visions() ===
~ return sanity < 50

// Функция: доступны ли скрытые пути безумия
=== function can_use_sanity_paths() ===
~ return sanity < 30

// Функция: разблокировать скрытый путь
=== function unlock_sanity_path(path) ===
{ not (unlocked_sanity_paths ? path):
    ~ unlocked_sanity_paths += path
    ~ return true
}
~ return false

// Функция: проверить разблокирован ли путь
=== function has_sanity_path(path) ===
~ return unlocked_sanity_paths ? path

// Функция: записать видение
=== function record_vision(vision_type) ===
~ total_visions_seen += 1
{ vision_type == 1:  // prophetic
    ~ prophetic_visions_seen += 1
}
{ vision_type == 0:  // false
    ~ false_visions_seen += 1
}
~ return total_visions_seen

// Функция: обновить осведомлённость NPC о безумии
=== function update_npc_sanity_awareness() ===
// Громов — замечает профессионально
{ sanity < 40 && npc_sanity_awareness_gromov < 2:
    ~ npc_sanity_awareness_gromov = 2
}
{ sanity < 60 && npc_sanity_awareness_gromov < 1:
    ~ npc_sanity_awareness_gromov = 1
}
// Вера — психиатр, замечает раньше
{ sanity < 50 && npc_sanity_awareness_vera < 2:
    ~ npc_sanity_awareness_vera = 2
}
{ sanity < 70 && npc_sanity_awareness_vera < 1:
    ~ npc_sanity_awareness_vera = 1
}
// Таня — эмоционально связана
{ sanity < 50 && npc_sanity_awareness_tanya < 1:
    ~ npc_sanity_awareness_tanya = 1
}
{ witnessed_breakdown_tanya:
    ~ npc_sanity_awareness_tanya = 3
}
// Серафим — понимает природу безумия
{ sanity < 30 && npc_sanity_awareness_serafim < 1:
    ~ npc_sanity_awareness_serafim = 1
}
// Клава — сплетница, видит внешние признаки
{ sanity < 40 && npc_sanity_awareness_klava < 2:
    ~ npc_sanity_awareness_klava = 2
}
~ return true

// Функция: NPC отказывается говорить из-за безумия
=== function npc_refuses_due_to_sanity(npc_name) ===
{ npc_name == "gromov":
    ~ return npc_sanity_awareness_gromov >= 3
}
{ npc_name == "vera":
    ~ return npc_sanity_awareness_vera >= 3
}
{ npc_name == "tanya":
    ~ return npc_sanity_awareness_tanya >= 3
}
{ npc_name == "klava":
    ~ return npc_sanity_awareness_klava >= 3
}
~ return false

// ─────────────────────────────────────────────────────────────────────────────
// TUNNEL: Пророческое видение (даёт подсказку)
// ─────────────────────────────────────────────────────────────────────────────
=== vision_prophetic(hint_text) ===
{ can_see_visions():
    # vision:prophetic
    # mood:horror
    # intensity:high
    
    Мир расплывается. Края зрения темнеют. # style:vision
    
    {hint_text} # style:vision
...
// ─────────────────────────────────────────────────────────────────────────────
// TUNNEL: Ложное видение (отвлекает)
// ─────────────────────────────────────────────────────────────────────────────
=== vision_false(false_text) ===
{ can_see_visions():
    # vision:false
    # mood:horror
    # intensity:medium
    
    Что-то мелькает на периферии зрения... # style:vision
    
    {false_text} # style:vision
    
    Вы моргаете — и ничего нет. Показалось? # style:thought
...
// ─────────────────────────────────────────────────────────────────────────────
// TUNNEL: Видение-воспоминание (Афганистан)
// ─────────────────────────────────────────────────────────────────────────────
=== vision_memory ===
{ can_see_visions() && afghan_flashbacks < 3:
    # vision:memory
    # mood:dark
    # intensity:high
    ...
    { shuffle:
    - 
        Горы. Песок в глазах. Крики. Выстрелы. # style:vision
        
        На мгновение вы снова ТАМ. В том ущелье. Кровь на руках. # style:vision
    -
        Лица. Мёртвые лица ваших товарищей. Они смотрят. # style:vision
        
        «Почему ты выжил, Сорокин? Почему ты?» # style:vision
    -
        Запах пороха. Запах горелого мяса. # style:vision
        
        Вы стоите над телом. Это... это был ребёнок. # style:vision
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// TUNNEL: Проверка и показ случайного видения
// ─────────────────────────────────────────────────────────────────────────────
=== maybe_show_vision ===
// 30% шанс видения при низком рассудке
{ can_see_visions() && RANDOM(1, 10) <= 3:
    { shuffle:
    -
        -> vision_prophetic("Красный свет. Круг из людей. Дверь открывается...") ->
    -
        -> vision_false("Тень в углу. Человеческий силуэт. Неподвижный.") ->
    -
        -> vision_false("Шёпот. Ваше имя. Снова и снова.") ->
    -
        -> vision_prophetic("Пещера. Алтарь. На алтаре — знакомое лицо...") ->
    -
        -> vision_memory ->
    }
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// СКРЫТЫЕ ПУТИ (доступны только при низком рассудке)
// ═══════════════════════════════════════════════════════════════════════════════

// Проверка: можно ли услышать голоса леса
=== function can_hear_forest_voices() ===
~ return sanity < 30 && (KeyEvents ? heard_voices)

// Проверка: можно ли видеть сквозь "завесу"
=== function can_see_through_veil() ===
~ return sanity < 20 && cult_awareness >= 15

// ═══════════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ — ФУНКЦИИ
// ═══════════════════════════════════════════════════════════════════════════════

// Проверка: достаточно ли доверия для эксклюзивной улики
=== function can_get_exclusive_clue_from(npc_name) ===
{ npc_name == "gromov":
    ~ return trust_gromov >= 70
}
{ npc_name == "vera":
    ~ return trust_vera >= 60
}
{ npc_name == "serafim":
    ~ return trust_serafim >= 80
}
{ npc_name == "tanya":
    ~ return trust_tanya >= 70
}
{ npc_name == "fyodor":
    ~ return trust_fyodor >= 50
}
~ return false

// Получить эксклюзивную улику от NPC
=== function give_exclusive_clue(clue) ===
{ not (obtained_exclusive_clues ? clue):
    ~ obtained_exclusive_clues += clue
...
// Проверка: есть ли эксклюзивная улика
=== function has_exclusive_clue(clue) ===
~ return obtained_exclusive_clues ? clue

// ─────────────────────────────────────────────────────────────────────────────
// КОНФЛИКТЫ ИНТЕРЕСОВ
// ─────────────────────────────────────────────────────────────────────────────

// Функция: рассказать одному NPC о другом (вызывает конфликт)
=== function tell_npc_about(informer, subject) ===
// Рассказать Громову о Вере
{ informer == "gromov" && subject == "vera":
    ~ told_gromov_about_vera = true
    ~ trust_vera -= 30
    ~ trust_gromov += 10
    ~ return true
}
// Рассказать Вере о культе (делает её уязвимой)
{ informer == "vera" && subject == "cult":
    ~ told_vera_about_cult = true
    ~ vera_danger_level += 1
    ~ trust_vera += 15
    ~ return true
}
// Рассказать Тане об опасности
{ informer == "tanya" && subject == "danger":
    ~ told_tanya_about_danger = true
    ~ tanya_danger_level += 1
    ~ trust_tanya += 10
    ~ return true
}
// Рассказать Серафиму о Чернове
{ informer == "serafim" && subject == "chernov":
    ~ told_serafim_about_chernov = true
    ~ trust_serafim += 20
    ~ return true
}
~ return false
...
// Принять просьбу NPC
=== function accept_request(npc_id) ===
~ active_request = npc_id
{ npc_id == 1:
    ~ trust_gromov += 5
}
{ npc_id == 2:
...
// Выполнить просьбу NPC
=== function complete_request(request) ===
{ not (completed_requests ? request):
    ~ completed_requests += request
    ~ active_request = 0
    // Бонусы за выполнение
    { request == req_gromov_wife || request == req_gromov_daughter:
        ~ trust_gromov += 25
        ~ understanding_gromov += 20
    }
    { request == req_vera_medicine || request == req_vera_patient:
        ~ trust_vera += 20
        ~ understanding_vera += 15
    }
    { request == req_serafim_icon || request == req_serafim_prayer:
        ~ trust_serafim += 20
        ~ understanding_serafim += 15
        ~ gain_sanity(5)
    }
    { request == req_tanya_lastday || request == req_tanya_photos:
        ~ trust_tanya += 25
...
// Проверка: выполнена ли просьба
=== function has_completed_request(request) ===
~ return completed_requests ? request

// ─────────────────────────────────────────────────────────────────────────────
// ПРЕДАТЕЛЬСТВА И СПАСЕНИЯ
// ─────────────────────────────────────────────────────────────────────────────

// Проверка: может ли NPC предать (низкое доверие + определённые условия)
=== function will_betray(npc_name) ===
{ npc_name == "gromov":
    ~ return trust_gromov < 20 && gromov_is_enemy
}
{ npc_name == "vera":
    ~ return trust_vera < 30 && not told_vera_about_cult
}
~ return false

// Проверка: спасёт ли NPC (высокое доверие)
=== function will_save(npc_name) ===
{ npc_name == "tanya":
...
{ npc_name == "fyodor":
    ~ return trust_fyodor >= 40 && (Relationships ? fyodor_ally)
}
{ npc_name == "gromov":
    ~ return trust_gromov >= 80 && gromov_is_ally
}
~ return false
...
// Луна растёт медленнее: ~7% в день (100 / 14 дней)
{ moon_phase < 100:
    ~ moon_phase = moon_phase + 7
}
{ moon_phase > 100:
    ~ moon_phase = 100
}
...
{ stopping:
    -
        ...
        — А, товарищ следователь! Как спалось? Завтрак готов — каша, яичница, компот. Всё свежее! # speaker:klava
...
{ stopping:
    ...
    -
        ...
        «...Виктор Андреевич...» # style:whisper # intensity:high
...
// К полнолунию 29 ноября — Дверь откроется полностью.
// Культ готовится. Но у Сорокина 15 дней на расследование.
//
...
{ stopping:
    ...
    -
        ...
        { ritual_countdown >= 10:
            {ritual_countdown} дней. {ritual_countdown} дней до полнолуния. # style:thought # intensity:high
    ...
    -
        ...
        { ritual_countdown <= 7 && ritual_countdown > 3:
            Меньше недели. # style:thought # intensity:high
    ...
    -
        ...
        «...скоро...» # style:vision # intensity:high
        «...скоро мы встретимся...» # style:vision # intensity:high
        «...Дверь откроется...» # style:vision # intensity:high
        
        { ritual_countdown <= 3:
            Осталось {ritual_countdown} {ritual_countdown == 1:день|дня}. Полнолуние близко. # style:thought # intensity:high
            
            Вы готовы? # style:thought # intensity:high
            
            Никто не бывает готов. # style:horror # intensity:high
        }
...
// Точка возврата из журнала (для универсального доступа)
// 0=ep1_night, 1=ep2_morning, 2=ep2_evening, 3=ep3, 4=ep4, 5=ep5
VAR journal_return_point = 0
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Раскрытие геологического прошлого Серафима
// ═══════════════════════════════════════════════════════════════════════════════

И ещё глубже копнув — вы узнаёте правду. # style:dramatic # intensity:high

Серафим Иванович Волков. До того как стать священником — был геологом. Участвовал в первых экспедициях 1953-54 годов. Один из первых, кто спустился в пещеры. # style:important

Он видел символы на стенах. Слышал первые голоса. Потерял друзей — троих товарищей, которые "заблудились" в лабиринтах и не вернулись. # style:dramatic # intensity:high

После того случая — ушёл из геологии. Принял сан. Остался в городе, чтобы... сторожить. Чтобы предупреждать тех, кто суётся слишком глубоко. # style:thought

{ not (CharacterSecrets ? serafim_geology):
    ~ CharacterSecrets += serafim_geology
    ~ understanding_serafim += 15
}
...
{ ritual_countdown >= 10:
    {ritual_countdown} дней до полнолуния. # style:thought # intensity:medium
}
{ ritual_countdown >= 5 && ritual_countdown < 10:
    {ritual_countdown} дней. # style:thought # intensity:medium
    
    { knows_deadline:
        Меньше десяти дней. Напряжение растёт. # style:thought # intensity:high
    }
}
{ ritual_countdown >= 2 && ritual_countdown < 5:
    {ritual_countdown} дня. # style:thought # intensity:high
    
    { knows_deadline:
        Скоро. Вы чувствуете — что-то нарастает. Напряжение в воздухе. # style:thought # intensity:high
    }
}
{ ritual_countdown == 1:
    ...
    { knows_deadline:
        Завтра полнолуние. Завтра — ритуал. Финал близок. # style:thought # intensity:high
    }
}
{ ritual_countdown == 0:
    ПОЛНОЛУНИЕ. ВРЕМЯ ПРИШЛО. # style:horror # intensity:high
}
...
— Следователь? — Она смотрит на ваше удостоверение. — Давненько к нам никто не заглядывал. # speaker:archivist

— Мне нужны документы по истории города. Довоенные. И раньше, если есть. # speaker:sorokin

Она качает головой. # style:action

— Раньше — мало что осталось. Война, пожар в пятьдесят первом... Но кое-что есть. Пойдёмте. # speaker:archivist
...
— Экспедиция? Какая экспедиция? # speaker:archivist
...
— Идёмте. — Её голос — шёпот. — Но это — между нами. # speaker:archivist
...
— Это не должно было сохраниться. — Она протягивает папку. — Я спрятала, когда в пятьдесят четвёртом приезжали люди из Москвы. Они забрали всё, что касалось... пещер. Но эту — не нашли. # speaker:archivist
...
* [Читать]
    ...
    — Теперь понимаете? — Мария Фёдоровна смотрит на вас. — Понимаете, почему люди исчезают? # speaker:archivist
...
* [Спросить, что она знает]
    ...
    — Много раз. — Она кивает. — С тех пор, как мой брат... — Она замолкает. # speaker:archivist
    
    — Ваш брат? # speaker:sorokin

    — Шестьдесят восьмой год. Работал на заводе. В секретном отделе. Однажды не пришёл домой. # speaker:archivist
...
— Пропавшие? — Мария Фёдоровна вздыхает. — Молодой человек, если бы вы знали, сколько людей пропало в этом городе... # speaker:archivist

Она достаёт толстую тетрадь. Самодельную, в клеёнчатой обложке. # style:action

— Я веду записи. Тридцать лет веду. Неофициально, конечно. Но... кто-то должен помнить. # speaker:archivist
...
— За тридцать лет? — Она листает тетрадь. — Двести сорок семь. Официально, конечно, меньше. Многих записали как уехавших, умерших от болезней... # speaker:archivist
...
* [Есть ли закономерность?]
    ...
    — Заметила. — Она кивает. — Больше всего исчезновений — в ноябре. В полнолуние. И... — Она понижает голос. — Все они слышали голоса. За несколько дней до исчезновения. # speaker:archivist
...
* [Кто расследовал?]
    ...
    — Было несколько следователей. До вас. — Мария Фёдоровна смотрит в сторону. — Один уехал через три дня. Сказал — дело закрыто. Второй... второй остался дольше. # speaker:archivist
    
    — И? # speaker:sorokin

    — Его нашли в лесу. Через неделю. Официально — сердечный приступ. # speaker:archivist
...
— Легенды? — Мария Фёдоровна усмехается. — Здесь всё — легенда. Весь город. # speaker:archivist

Она садится в кресло. Жестом приглашает вас сесть напротив. # style:action

— Мой дед рассказывал... Он родился здесь, в деревне, которая была до города. Когда-то — давно, ещё при царе — сюда пришли геологи. Искали руду. # speaker:archivist

— Экспедиция Корнилова? # speaker:sorokin

— Раньше. Гораздо раньше. Ещё при Екатерине. Нашли что-то в горах. Что-то... — Она замолкает. — Дед говорил — "нехорошее место". Местные знали. Не ходили туда. # speaker:archivist
...
— Потом — революция. Война. Сталин. Завод построили в пятьдесят втором. А под ним — лаборатории. Секретные. # speaker:archivist

Она наклоняется ближе. # style:action # intensity:medium

— Дед умер в пятьдесят шестом. Но перед смертью сказал мне: "Маша, они открыли Дверь. Нельзя было открывать. Теперь — поздно." # speaker:archivist
...
* [Какую Дверь?]
    ...
    — Не знаю. — Она качает головой. — Но... — Она достаёт из кармана маленький свёрток. Разворачивает. # speaker:archivist
    
    Камень. Чёрный, гладкий. С выцарапанным символом — три линии, сходящиеся к центру круга. # style:action

    — Это нашли рядом с дедом. После смерти. В руке зажал. # speaker:archivist
...
— Вы не такой, как другие следователи. — Её голос — тихий. — Вы ищете правду. По-настоящему. # speaker:archivist

— Да. # speaker:sorokin

— Тогда будьте осторожны. — Она берёт вас за руку. Её пальцы — холодные, как лёд. — Правда в этом городе... она убивает. Медленно. Или быстро. Но убивает. # speaker:archivist # intensity:high

+ [Поблагодарить и уйти]
    — Спасибо, Мария Фёдоровна. Вы очень помогли. # speaker:sorokin

    — Помогла... — Она усмехается. — Может быть. А может — погубила. Время покажет. # speaker:archivist
...
* [Спросить, как защититься]
    ...
    — Не слушайте голоса. Что бы они ни говорили. Что бы ни обещали. Это — ловушка. Всегда ловушка. # speaker:archivist # intensity:high
...
ДЕНЬ {current_day}/15 • {time_of_day == 0:Утро}{time_of_day == 1:День}{time_of_day == 2:Вечер}{time_of_day == 3:Ночь}
...
// ═══════════════════════════════════════════════════════════════════════════════
// 📋 ЖУРНАЛ СЛЕДОВАТЕЛЯ — УНИВЕРСАЛЬНЫЙ ВОЗВРАТ
// ═══════════════════════════════════════════════════════════════════════════════
=== journal_return ===
// Возврат к точке, откуда был вызван журнал
{ journal_return_point == 0:
    -> ep1_night_choice
}
{ journal_return_point == 1:
    -> episode2_morning
}
{ journal_return_point == 2:
    -> episode2_evening
}
{ journal_return_point == 3:
    -> episode3_intro
}
{ journal_return_point == 4:
    -> episode4_morning
}
{ journal_return_point >= 5:
    // Дни 5-15 — расширенный режим
    -> extended_day_hub
}
// Fallback на ep1
-> ep1_night_choice
...
// ═══════════════════════════════════════════════════════════════════════════════
// 📋 ЖУРНАЛ СЛЕДОВАТЕЛЯ — ПОБОЧНЫЕ РАССЛЕДОВАНИЯ
// ═══════════════════════════════════════════════════════════════════════════════
=== journal_sidequests ===
# mood: investigation
# ui: journal
...
// ═══ АНОНИМНЫЕ ПИСЬМА ═══
{ active_sidequests ? sq_letters_started:
    ✉️ АНОНИМНЫЕ ПИСЬМА: В процессе
    • Получено писем: {letters_received}
    • Статус: Получено первое письмо, ждём развития
}

{ active_sidequests ? sq_letters_trusted:
    ✉️ АНОНИМНЫЕ ПИСЬМА: В процессе
    • Получено писем: {letters_received}
    • Статус: Решено следовать совету — идти к шахте вечером
}

{ active_sidequests ? sq_letters_ignored:
    ✉️ АНОНИМНЫЕ ПИСЬМА: Отклонено
    • Получено писем: {letters_received}
    • Статус: Письма проигнорированы как провокация
}

{ active_sidequests ? sq_letters_solved:
    ✉️ АНОНИМНЫЕ ПИСЬМА: ✅ Завершено
    • Автор: Николай Семёнович (бывший член культа)
    • Итог: Получены документы о культе, имена, схемы
}

// ═══ РАСШИРЕННЫЕ ПОБОЧНЫЕ КВЕСТЫ ═══
{ active_extended_sq ? esq_dogs:
    🐕 ПРОПАВШИЕ СОБАКИ:
    { dogs_truth_known: ✅ Завершено — правда раскрыта }
    { not dogs_truth_known: ⏳ В процессе — найдено собак: {dogs_found}/5 }
}

{ active_extended_sq ? esq_ghost:
    👻 ПРИЗРАК ШАХТЁРА:
    { ghost_identity_known: ✅ Завершено — личность установлена }
    { not ghost_identity_known: ⏳ В процессе — встреч с призраком: {ghost_sightings} }
}

{ active_extended_sq ? esq_diary:
    📔 ДНЕВНИК УЧИТЕЛЯ:
    { diary_complete: ✅ Завершено — дневник собран }
    { not diary_complete: ⏳ В процессе — найдено страниц: {diary_pages_found}/7 }
}

{ active_extended_sq ? esq_radio:
    📻 ГОЛОСА ПО РАДИО:
    { radio_source_found: ✅ Завершено — источник найден }
    { not radio_source_found: ⏳ В процессе — услышано сообщений: {radio_messages_heard} }
}

{ active_extended_sq ? esq_graves:
    ⚰️ БЕЗЫМЯННЫЕ МОГИЛЫ:
    { mass_grave_found: ✅ Завершено — массовое захоронение найдено }
    { not mass_grave_found: ⏳ В процессе — опознано могил: {graves_identified} }
}

{ active_sidequests ? sq_nurse_started || active_sidequests ? sq_nurse_helped || active_sidequests ? sq_nurse_betrayed:
    👩‍⚕️ ПРОСЬБА МЕДСЕСТРЫ:
    { active_sidequests ? sq_nurse_started: В процессе }
    { active_sidequests ? sq_nurse_helped: ✅ Помогли }
    { active_sidequests ? sq_nurse_betrayed: ❌ Предали }
}

// ═══ КВЕСТЫ РЫНКА ═══
{ quest_masha_documents || quest_masha_documents_done:
    📄 ДОКУМЕНТЫ ВИКТОРА МОРОЗОВА:
    { quest_masha_documents && not quest_masha_documents_done: ⏳ В процессе — найти спрятанные документы мужа тёти Маши }
    { quest_masha_documents_done: ✅ Завершено — документы найдены }
}

{ quest_lyuda_medicine || quest_lyuda_medicine_done:
    💊 ЛЕКАРСТВО ДЛЯ МАМЫ ЛЮДЫ:
    { quest_lyuda_medicine && not quest_lyuda_medicine_done: ⏳ В процессе — достать Церебролизин (больница или Вера) }
    { quest_lyuda_medicine_done: ✅ Завершено — лекарство передано }
}

{ quest_grisha_violin || quest_grisha_violin_done:
    🎻 СКРИПКА ГРИШИ:
    { quest_grisha_violin && not quest_grisha_violin_done: ⏳ В процессе — найти скрипку (комиссионка или дом Астахова) }
    { quest_grisha_violin_done: ✅ Завершено — скрипка возвращена }
}

{ quest_kolka_warning && not quest_kolka_completed:
    🌊 ТАЙНА ОЗЕРА: ⏳ В процессе
    
    Колька-рыбак предложил показать озеро ночью в полнолуние.
    
    📋 Что нужно собрать:
    { quest_kolka_has_silver: ✅ }{ not quest_kolka_has_silver: ⬜ } Серебро — попросить крестик у Серафима
    { quest_kolka_has_fire: ✅ }{ not quest_kolka_has_fire: ⬜ } Огонь — найти факел или спички
    { quest_kolka_has_believer: ✅ }{ not quest_kolka_has_believer: ⬜ } Верующий — уговорить Серафима пойти с вами
    
    { quest_kolka_has_silver && quest_kolka_has_fire && quest_kolka_has_believer:
        ✨ Всё готово! Найдите Кольку на рынке и идите к озеру.
    }
}

{ quest_kolka_completed:
    🌊 ТАЙНА ОЗЕРА: ✅ Завершено
    • Вы посетили озеро с Колькой и узнали его тайну
}

// ═══ ИНТЕРЛЮДИИ ═══
{ interlude_nightmare_1_played || interlude_nightmare_2_played || interlude_nightmare_3_played:
    
    💤 НОЧНЫЕ КОШМАРЫ:
    { interlude_nightmare_1_played: 
        • Кошмар 1 — { nightmares_won > 0: Побеждён (Видение алтаря) }{ nightmares_lost > 0 && nightmares_won == 0: Сбежали }
    }
    { interlude_nightmare_2_played: • Кошмар 2 — Пройден }
    { interlude_nightmare_3_played: • Кошмар 3 — Пройден }
    
    📊 Статистика: {nightmares_won} побед, {nightmares_lost} побегов
}

// ═══ ПУСТО ═══
{ not (active_sidequests ? sq_letters_started) && not (active_sidequests ? sq_letters_trusted) && not (active_sidequests ? sq_letters_ignored) && not (active_sidequests ? sq_letters_solved) && not interlude_nightmare_1_played && LIST_COUNT(active_extended_sq) == 0 && not quest_masha_documents && not quest_lyuda_medicine && not quest_grisha_violin && not quest_kolka_warning:
    🔍 Побочных расследований пока нет.
    
    Внимательно изучайте окружение — иногда важные зацепки появляются неожиданно.
}
...
// ═══ КАТЕГОРИЯ D: ИСТОРИЯ ═══
{ CluesD ? expedition_1890 || CluesD ? serafim_legends || CluesD ? church_symbols || CluesD ? old_mine_secret:
    📜 ИСТОРИЯ:
    { CluesD ? expedition_1890: • 🗿 Экспедиция 1890 — история началась давно }
    { CluesD ? serafim_legends: • ⛪ Легенды Серафима — древние предания о тёмных силах }
    { CluesD ? church_symbols: • ✝️ Символы в церкви — языческие знаки под православными }
    { CluesD ? old_mine_secret: • ⛏️ Тайна старой шахты — шахтёры нашли древние ходы под землёй }
}

// ═══ КАТЕГОРИЯ E: ПОКАЗАНИЯ И СВИДЕТЕЛЬСТВА ═══
{ CluesE ? fyodor_map || CluesE ? vera_research || CluesE ? gromov_confession || CluesE ? zorin_warning_letters || CluesE ? zorin_night_photos || CluesE ? zorin_hidden_notes || CluesE ? klava_testimony || CluesE ? old_photos:
    🗣️ ПОКАЗАНИЯ И НАХОДКИ:
    ...
    { CluesE ? zorin_warning_letters: • ✉️ Письма Зорину — анонимные предупреждения }
    { CluesE ? zorin_night_photos: • 📸 Ночные фото Зорина — ритуалы в лесу }
    { CluesE ? zorin_hidden_notes: • 📓 Тайные записи Зорина — скрытая тетрадь о подземельях завода }
    { CluesE ? klava_testimony: • 👵 Показания Клавы — свидетельства из столовой }
    { CluesE ? old_photos: • 🖼️ Старые фотографии — лица из прошлого }
}

// ═══ ЭКСКЛЮЗИВНЫЕ УЛИКИ (требуют высокое доверие NPC) ═══
{ LIST_COUNT(obtained_exclusive_clues) > 0:
    ⭐ ЭКСКЛЮЗИВНЫЕ УЛИКИ:
    { obtained_exclusive_clues ? excl_gromov_key: • 🔑 Ключ Громова — архив милиции + закрытые дела }
    { obtained_exclusive_clues ? excl_vera_medcards: • 🏥 Медкарты проекта "Эхо" — эксперименты над людьми }
    { obtained_exclusive_clues ? excl_serafim_catacombs: • ⛪ Карта катакомб — древние ходы под церковью }
    { obtained_exclusive_clues ? excl_tanya_diary: • 📓 Дневник Зорина — имена членов культа, схемы }
    { obtained_exclusive_clues ? excl_fyodor_safe_path: • 🛤️ Безопасный путь — обход ловушек и охраны }
    { obtained_exclusive_clues ? excl_chernov_letters: • 📜 Письма Чернова к Зорину — связь учёного с культом }
}
...
~ update_npc_sanity_awareness()

{ infection_level >= 40:
    Его глаза сужаются. Он видит что-то в вас — и это ему не нравится. # style:thought # intensity:medium
}

// РАСШИРЕННАЯ СИСТЕМА: Реакция на рассудок
{ sanity < 40 && npc_sanity_awareness_gromov >= 2:
    Громов замечает дрожь ваших рук. Круги под глазами. Взгляд, который слишком долго задерживается на тенях в углах комнаты. # style:thought # intensity:medium
    
    — Вы... здоровы, товарищ следователь? — Его голос — осторожный. Профессиональный. — Выглядите неважно. # speaker:gromov # intensity:medium
    
    { sanity < 30:
        Вы видите, как он незаметно отодвигает бутылку подальше. Как его рука скользит под стол — туда, где наверняка лежит пистолет. # style:horror # intensity:high
        
        Он боится вас. Или того, что он видит В вас. # style:thought # intensity:high
    }
}

{ sanity < 60 && sanity >= 40 && npc_sanity_awareness_gromov >= 1:
    — Долгая дорога, да? — Громов смотрит на вас внимательно. — Может, отдохнёте сначала? В гостинице номер готов. # speaker:gromov # intensity:low
...
// ═══════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Просьба Громова
// ═══════════════════════════════════════════════════════════════════════════
{ CharacterSecrets ? gromov_daughter && trust_gromov >= 30 && not (completed_requests ? req_gromov_daughter) && active_request == 0:
    -> ep1_gromov_request ->
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Просьба Громова (tunnel)
// ═══════════════════════════════════════════════════════════════════════════════
=== ep1_gromov_request ===

Громов замирает у двери. Его рука — на ручке. Он не оборачивается. # style:atmosphere # intensity:medium

— Сорокин... — Его голос — ломается. — Вы сказали, что видите. Что умеете... находить. # speaker:gromov # intensity:medium

Долгая пауза. # style:dramatic

— Аня. Моя Аня. Девятнадцать лет. — Он поворачивается. Его глаза — красные, влажные. — Я не прошу найти её живой. Я не идиот. Но... хоть что-то. Хоть знать, ЧТО случилось. # speaker:gromov # intensity:high

* [Согласиться помочь]
    — Я постараюсь, Степан Петрович. # speaker:sorokin
    
    Громов смотрит на вас. Кивает. В его глазах — что-то похожее на надежду. Впервые за долгое время. # style:atmosphere # intensity:medium
    
    ~ accept_request(1)
    ~ NPCRequests += req_gromov_daughter
    
    # quest
    Принято: найти информацию о дочери Громова (Анна Громова, пропала 19 лет назад)
    
    ->->
    
* [Отказаться]
    — У меня есть своё расследование, майор. # speaker:sorokin
    
    Громов кивает. Тяжело, устало. Что-то гаснет в его глазах. # style:atmosphere # intensity:medium
...
// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма — Tunnel для первого письма
// ═══════════════════════════════════════════════════════════════════════════════
=== ep1_first_letter ===

На тумбочке — конверт. Белый, без марки, без обратного адреса. # style:important # intensity:medium

Только ваше имя. Напечатано на машинке. # style:thought

Странно. Вы только приехали. Кто знал, какой номер вам дадут? # style:thought # intensity:medium

* [Открыть конверт]
    Вы вскрываете конверт. Внутри — листок. Тоже машинопись. # style:action
    
    "Товарищ Сорокин.
    
    Вас прислали найти пропавших. Вы их не найдёте. Никто не находит.
    
    Но вы можете узнать ПРАВДУ. Если будете слушать.
    
    Не доверяйте официальным лицам. Особенно — тем, кто носит красное.
    
    Больница. Архив. Там — начало.
    ...
    # sidequest
    ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма начато
    
    Кто этот "друг"? Провокатор? Или действительно кто-то, кто хочет помочь? # style:thought # intensity:high
    
    ->->
    
* [Игнорировать]
    Провокация. Классическая провокация. # style:thought
    
    Вы убираете конверт в карман. Может, пригодится позже. # style:action
...
// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма (письмо #1)
// ═══════════════════════════════════════════════════════════════════════════════
{ letters_received == 0:
    -> ep1_first_letter ->
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Использование лекарства для восстановления рассудка
// ═══════════════════════════════════════════════════════════════════════════════
* { inventory ? item_medicine && sanity < 70 } [Принять успокоительное]
    Вы достаёте пузырёк, который дала Вера. Отмеряете несколько капель. # style:action
    
    Горьковатый вкус. Травы. Что-то ещё — химическое, но не неприятное. # style:atmosphere
    
    Через минуту — тепло разливается по телу. Напряжение отступает. Голоса — если они были — замолкают. # style:atmosphere # intensity:medium
    ...
    # effect
    Рассудок восстановлен: +15% (лекарство использовано)
    
    Голова проясняется. Мир обретает чёткость. # style:thought # intensity:medium
...
// РАСШИРЕНИЕ: Рынок — вечерний рынок ещё работает
+ [На рынок — узнать, чем живёт город]
    Вечерний рынок ещё работает — торговцы сворачиваются, но кое-кто остался. # style:atmosphere
...
=== ep1_found_cache ===
// ИСПРАВЛЕНО: Можно открыть с отмычкой (тихо) или без (громко)

* { has_item(item_lockpick) } [Открыть отмычкой — тихо и аккуратно]
    Отмычки Фёдора делают своё дело. Тихий щелчок — и доска поддаётся. # style:action
    -> ep1_open_cache_silent

* [Поддеть силой — рискованно, но быстро]
    Вы находите щель и давите. Дерево скрипит — громко, слишком громко в ночной тишине. # style:action # intensity:medium
    
    Кто-то мог услышать. # style:thought # intensity:high
    ~ city_reputation = city_reputation - 3
    
    -> ep1_open_cache_loud

* [Оставить на потом]
    Нужен инструмент. Или план получше. # style:thought # intensity:low
    -> ep1_night_choice
...
Вы аккуратно закрываете тайник. Никто не узнает, что вы здесь были. # style:thought
...
За стеной — шорох. Шаги? Вы замираете. # style:atmosphere # intensity:high

Тишина. Показалось? # style:thought # intensity:medium
...
// РАСШИРЕННАЯ СИСТЕМА: Клава замечает состояние рассудка
~ update_npc_sanity_awareness()

Клава за стойкой поднимает голову. Её глаза — испуганные. # style:thought # intensity:medium

{ npc_sanity_awareness_klava >= 2:
    Она смотрит на вас — и отшатывается. Что она видит? Что читает на вашем лице? # style:thought # intensity:medium
    
    — Товарищ следователь? Вы... вы в порядке? Может, чаю горячего? — Её голос дрожит. # speaker:klava
- else:
...
{ npc_sanity_awareness_klava >= 2:
    — Не надо! — Она хватает вас за рукав. — Ночью... ночью нельзя. Они ходят ночью. # speaker:klava # intensity:high
    
    Вы мягко освобождаете руку. # style:action
    
    — Кто — они? # speaker:sorokin
    
    Но она не отвечает. Только качает головой, как испуганная птица. # style:thought # intensity:medium
- else:
...
// РАСШИРЕННАЯ СИСТЕМА ВИДЕНИЙ: Шанс видения на улице ночью
-> maybe_show_vision ->
...
«Серафим Иванович Волков (старший). 1889-1952. Хранитель». # style:document # intensity:high

    Отец нынешнего священника? Или просто предшественник с тем же именем? # style:thought # intensity:medium
...
// РАСШИРЕННАЯ СИСТЕМА ВИДЕНИЙ: Пророческое видение у границы леса
{ can_see_visions():
    -> vision_prophetic("Красный свет между деревьями. Фигуры в капюшонах. Они стоят кругом. В центре — алтарь. На алтаре — кто-то знакомый...") ->
    
    { can_use_sanity_paths() && not has_sanity_path(path_voices_cave):
        И тогда вы слышите это. Голос. Не снаружи — внутри. # style:horror # intensity:high
        
        «Вход. За третьей сосной слева. Камни образуют арку. Мы ждём тебя, Сорокин.» # style:whisper # intensity:high
        
        ~ unlock_sanity_path(path_voices_cave)
        
        # clue
        Скрытый путь разблокирован: Голоса указали тайный вход в пещеры
    }
}
...
— Инженерный корпус, комната 214. — Пауза. — Она... хорошая девочка. Не обижайте её. # speaker:guard
...
// ═══════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Эксклюзивная улика от Тани
// ═══════════════════════════════════════════════════════════════════════════
{ can_get_exclusive_clue_from("tanya") && not has_exclusive_clue(excl_tanya_diary):
    Таня замирает. Её взгляд — изучающий. Решающий что-то для себя. # style:atmosphere # intensity:medium
    
    — Вы... вы не такой, как другие. Я вижу. — Она понижает голос до шёпота. # speaker:tanya # intensity:medium
    
    Из-под стола она достаёт толстую тетрадь. Обёрнутую в клеёнку. # style:action
    
    — Это его настоящий дневник. Не те записки, что я показала раньше. — Её руки дрожат. — Здесь всё. Всё, что он знал. Я хранила его... на случай, если найдётся кто-то, кому можно доверять. # speaker:tanya # intensity:high
    
    ~ give_exclusive_clue(excl_tanya_diary)
    
    # clue
    ЭКСКЛЮЗИВНАЯ УЛИКА: Дневник Зорина (полная версия)
    
    Дневник содержит: имена членов культа, схемы подземелий, даты ритуалов, и... координаты Чернова. # style:important # intensity:high
...
— Я... Я видела его. Зорина. # speaker:witness

— Где именно? # speaker:sorokin

Пауза. Она оглядывается.

— Они сказали — скажи, что видела. А если нет... # speaker:witness

— Кто — они? # speaker:sorokin

— Мужчина в штатском. Сказал — от милиции. # speaker:witness
...
— Я никого не видела тем вечером. Клянусь. Простите меня. # speaker:witness
...
// РАСШИРЕННАЯ СИСТЕМА РАССУДКА: Скрытый путь через Серафима
{ can_use_sanity_paths() && not has_sanity_path(path_dead_speak):
    Серафим останавливается. Смотрит на вас — долго, пристально. # style:atmosphere # intensity:medium
    
    — Вы... видите. — Это не вопрос. — Границы истончаются для вас. # speaker:serafim # intensity:high
    
    Он достаёт из-за иконы старый ключ. Ржавый, древний. # style:action
    
    — Под церковью — катакомбы. Старые, ещё языческие. Там... там можно услышать тех, кто ушёл. — Его голос дрожит. — Мёртвые говорят. Если слушать. # speaker:serafim # intensity:high
    
    ~ unlock_sanity_path(path_dead_speak)
    ~ give_exclusive_clue(excl_serafim_catacombs)
    
    # clue
    СКРЫТЫЙ ПУТЬ: Ключ от катакомб под церковью
    
    Катакомбы. Мёртвые. Голоса. # style:thought # intensity:high
    
    Это безумие. Или... единственный способ найти правду? # style:thought # intensity:high
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ОПЦИОНАЛЬНАЯ ИНТЕРЛЮДИЯ: Ночной кошмар
// ═══════════════════════════════════════════════════════════════════════════════
{ not interlude_nightmare_1_played:
    * [💤 Интерлюдия: Ночной кошмар]
        -> interlude_nightmare_1
        
    * [⏭️ Пропустить — следующий день...]
        -> episode2_intro
- else:
    * [Следующий день...]
        -> episode2_intro
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИНТЕРЛЮДИЯ 1: НОЧНОЙ КОШМАР
// ═══════════════════════════════════════════════════════════════════════════════
=== interlude_nightmare_1 ===

~ interlude_nightmare_1_played = true
// Интеграция с системой афганских флэшбеков
~ afghan_flashbacks += 1
~ AfghanMemories += memory_ambush
...
ИНТЕРЛЮДИЯ: НОЧНОЙ КОШМАР # style:title # intensity:high

Вы спите. # style:atmosphere

Но это — не сон. Не обычный сон. # style:horror # intensity:medium
...
Жара. Пыль. Горы на горизонте — жёлтые, выжженные солнцем. # style:vision # intensity:high

Афганистан. # style:title # intensity:high

Вы снова ТАМ. # style:horror # intensity:high

{ afghan_flashbacks >= 2:
    Знакомое место. Слишком знакомое. # style:thought
}

Конвой. Три БТР. Вы — в переднем, за пулемётом. Рядом — Лёша Петров, двадцать один год, из Саратова, показывает фотографию невесты. # style:vision # intensity:medium

— Смотри, командир, красивая, да? Светка. Ждёт меня. — Его улыбка — широкая, наивная. # speaker:soldier # style:vision

Впереди — ущелье. Узкое. Скалы нависают с обеих сторон. # style:atmosphere # intensity:high

Плохое место. # style:thought # intensity:medium

* [Приказать остановиться]
    — Стой! — кричите вы. — Проверить! # speaker:sorokin # style:vision
    
    Но никто не слышит. Или не хочет слышать. # style:horror
    
    БТР въезжает в ущелье. # style:vision # intensity:high
    
    -> nightmare_1_ambush

* [Промолчать (как было тогда)]
    Вы молчите. Как тогда. # style:thought
    
    Конвой движется вперёд. В пасть ущелья. # style:vision # intensity:high
...
Взрыв. # style:horror # intensity:high # effect:shake

Передний БТР подбрасывает вверх, как игрушку. Огонь. Крики. # style:vision # intensity:high

— Засада! ЗАСАДА! — кричит кто-то. # speaker:soldier # style:vision

Пули — градом. Со всех сторон. Камни крошатся, рикошеты визжат. # style:horror # intensity:high

Лёша — тот, с фотографией — падает. Его глаза — удивлённые. Красное пятно расплывается на груди. # style:horror # intensity:high

— Командир... — шепчет он. — Светка... # speaker:soldier # style:vision # intensity:high

И замолкает. # style:horror # intensity:high

-> lose_sanity_safe(3) ->

* [Сражаться]
    Вы стреляете. Перезаряжаете. Стреляете снова. # style:vision # intensity:high
    
    Тела падают — и свои, и чужие. # style:horror
    
    -> nightmare_1_forest

* [Бежать]
    Вы бежите. Как трус. Как крыса. # style:vision # intensity:high
    
    За спиной — крики умирающих. # style:horror
...
И вдруг — всё меняется. # style:dramatic # intensity:high

Горы исчезают. Пустыня становится... лесом. # style:vision # intensity:high

Красный лес. # style:horror # intensity:high

Сосны — чёрные, как обугленные. Небо — красное, как кровь. Под ногами — не песок, а мёрзлая земля. # style:vision # intensity:high

— Сорокин... # style:whisper # intensity:high

Голос. Лёши? Нет. Другой. Много голосов. # style:horror # intensity:medium

— Почему ты выжил? # style:whisper # intensity:high

— Почему не они? # style:whisper # intensity:high

— ПОЧЕМУ ТЫ? # style:whisper # intensity:high

{ sanity < 50:
    Фигуры между деревьями. Десятки. Лица — знакомые. Мёртвые. # style:horror # intensity:high
    
    Все, кого вы не смогли спасти. # style:vision # intensity:high
}

* [Ответить: "Я не знаю"]
    — Я не знаю! — кричите вы в темноту. — Я НЕ ЗНАЮ! # speaker:sorokin # style:vision # intensity:high
    
    Молчание. Долгое, давящее. # style:atmosphere # intensity:high
    
    — Хорошо. — Голос — другой. Спокойный. Древний. — Честный ответ. # style:whisper # intensity:medium
    
    -> nightmare_1_choice

* [Ответить: "Чтобы найти правду"]
    — Чтобы найти правду! — Ваш голос — твёрдый. Злой. — Чтобы остановить это! # speaker:sorokin # style:vision # intensity:high
    
    Смех. Тихий, шелестящий. # style:horror # intensity:medium
    
    — Правда? — Голос наполняется иронией. — Ты уверен, что хочешь её знать? # style:whisper # intensity:high
    
    -> nightmare_1_choice

* [Молчать]
    Вы молчите. Сжимаете кулаки. # style:action
    
    — Молчание — тоже ответ. — Голос звучит... одобрительно? — Ты сильнее, чем думаешь. # style:whisper # intensity:medium
...
Перед вами — развилка. # style:vision # intensity:high

Слева — свет. Тёплый, жёлтый. Запах хлеба, звуки музыки. Дом? Детство? # style:atmosphere # intensity:medium

Справа — темнота. Холод. Но там — что-то важное. Вы чувствуете. # style:horror # intensity:medium

— Выбирай. — Голос — повсюду. — Покой или знание. # style:whisper # intensity:high

* [Налево — к свету (побег)]
    Вы идёте к свету. # style:action
    
    Тепло обнимает вас. Голоса стихают. Мир становится мягким, уютным... # style:atmosphere # intensity:medium
    
    И вы просыпаетесь. # style:dramatic
    
    ~ nightmares_lost += 1
    -> lose_sanity_safe(5) ->
    
    # nightmare
    Вы сбежали из кошмара. Рассудок -5.
    
    -> nightmare_1_end

* [Направо — в темноту (противостояние)]
    Вы идёте в темноту. # style:action # intensity:high
    
    Холод усиливается. Темнота сгущается. # style:horror # intensity:high
    
    И тогда — вы видите. # style:dramatic # intensity:high
    
    -> nightmare_1_revelation
...
// Интеграция с системой видений
~ record_vision(1)  // 1 = prophetic

Алтарь. # style:vision # intensity:high

Каменный, древний, покрытый рунами. В центре — чаша. Полная чем-то тёмным. # style:horror # intensity:high

Вокруг — фигуры в капюшонах. Их лица — невидимы. Но вы знаете: они реальны. # style:vision # intensity:high

Один из них — поворачивается. Снимает капюшон. # style:dramatic # intensity:high

Лицо — знакомое. Но вы не можете вспомнить, где видели его. # style:thought # intensity:high

— Ты придёшь, — говорит он. — Добровольно или нет. Все приходят. # speaker:cultist # style:vision # intensity:high

— Дверь ждёт. # speaker:cultist # style:horror # intensity:high

За алтарём — что-то... открывается. # style:horror # intensity:high

Красный свет. Бесконечность. Глаза — тысячи глаз — смотрят на вас. # style:horror # intensity:high # effect:glitch

И вы — кричите. # style:horror # intensity:high

... # style:dramatic

Вы просыпаетесь. # style:atmosphere

Сердце колотится. Простыня — мокрая от пота. Но... # style:thought # intensity:medium

Вы видели. Вы ЗНАЕТЕ. # style:important # intensity:high

~ nightmares_won += 1
~ gain_sanity(5)

# nightmare
Вы победили кошмар. Рассудок +5. Получена символическая подсказка.

// Интеграция с системой улик — добавляем видение как улику
{ not (CluesD ? serafim_legends):
    // Видение о ритуале связано с древними легендами
    ~ CluesD += serafim_legends
    ~ sync_evidence_count()
}
...
# clue
ВИДЕНИЕ: Алтарь в пещерах. Дверь. Лицо под капюшоном — вы узнаете его позже.
...
{ nightmares_won > 0:
    Утро. Серый свет сочится сквозь шторы. # style:atmosphere
    
    Вы лежите, глядя в потолок. Образы из сна — всё ещё перед глазами. # style:thought # intensity:medium
    
    Это было не просто сновидение. Это была... подсказка? Предупреждение? # style:thought # intensity:high
    
    Вы встаёте. Голова — ясная. Странно ясная для того, что вы пережили. # style:atmosphere
- else:
    Утро. Серый свет. Головная боль. # style:atmosphere
    
    Вы лежите, пытаясь вспомнить сон. Но образы ускользают — как вода сквозь пальцы. # style:thought # intensity:medium
    
    Что-то важное было там. Что-то, что вы упустили. # style:thought # intensity:high
    
    Вы встаёте. Тяжело. День предстоит долгий. # style:atmosphere
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма — Tunnel для второго письма
// ═══════════════════════════════════════════════════════════════════════════════
=== ep2_second_letter ===

Под дверью — ещё один конверт. Белый. Без марки. # style:important # intensity:medium

Вы поднимаете. Открываете. # style:action

"Вы послушали. Хорошо.

Сегодня — идите в лес. К старой шахте. Вечером, после заката.

Там найдёте то, что ищете. Или то, что ищет ВАС.

Выбор за вами. Доверьтесь — или нет.
...
# sidequest
Анонимные письма: Письмо #2 — Приглашение к шахте

* [Следовать совету (рискованно)]
    Риск. Но... это единственная реальная зацепка. # style:thought
    
    ~ letters_followed += 1
    // Переход состояния: started → trusted
    ~ active_sidequests -= sq_letters_started
    ~ active_sidequests += sq_letters_trusted
    
    Вечером — к шахте. # style:thought # intensity:medium
    
    ->->
    
* [Игнорировать — ловушка]
    Классическая провокация. Заманить в лес, ночью, одного. # style:thought
    
    // Переход состояния: started → ignored
    ~ active_sidequests -= sq_letters_started
    ~ active_sidequests += sq_letters_ignored
    
    Нет. Вы не пойдёте. # style:thought
...
// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма (письмо #2)
// ═══════════════════════════════════════════════════════════════════════════════
{ (active_sidequests ? sq_letters_started) && letters_received == 1:
    -> ep2_second_letter ->
}
...
// ═══════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Конфликт интересов
// ═══════════════════════════════════════════════════════════════════════════
* { actions_today < 3 && (MetCharacters ? vera) && (CluesE ? vera_research) && not told_gromov_about_vera } [К Громову — рассказать о Вере]
    -> ep2_tell_gromov_about_vera

// ═══════════════════════════════════════════════════════════════════════════
// НОВАЯ ЛОКАЦИЯ: Квартира Зориных (требует доверие Тани >= 40)
// ═══════════════════════════════════════════════════════════════════════════
* { actions_today < 3 && trust_tanya >= 40 && (MetCharacters ? tanya) && not zorin_apt_searched } [К Тане — попросить показать квартиру отца]
    Вы находите Таню на заводе. # style:action
    
    — Татьяна Алексеевна, мне нужно осмотреть квартиру вашего отца. # speaker:sorokin
    
    Она бледнеет. Но — кивает. # style:atmosphere # intensity:medium
    
    — Хорошо. Я... я покажу. После работы. В шесть. # speaker:tanya
    
    ~ zorin_apt_unlocked = true
    
    -> ep2_zorin_apartment

// ИСПРАВЛЕНИЕ: Новая локация — Библиотека (открывает hidden_archive)
* { actions_today < 3 && not (SecretLocations ? hidden_archive) } [В библиотеку — поискать архивы]
    -> episode2_library

// РАСШИРЕНИЕ: Рынок — новые NPC, слухи, атмосфера
* { actions_today < 3 && (time_of_day == 0 || time_of_day == 1) } [На рынок — послушать, о чём говорит город]
    -> location_market_day1to5
...
// ═══════════════════════════════════════════════════════════════════════════════
// НОВАЯ СЦЕНА: Библиотека — разблокирует hidden_archive и артефакты
// ═══════════════════════════════════════════════════════════════════════════════
...
Городская библиотека. # style:title # intensity:medium

Небольшое здание на площади Ленина. Красный кирпич, облупившаяся вывеска "БИБЛИОТЕКА им. ГОРЬКОГО". # style:atmosphere

Внутри — тишина. Пыль в лучах света из высоких окон. Стеллажи уходят в глубину зала — тёмные, массивные, из старого дерева. # style:atmosphere # intensity:medium

За столом — пожилая женщина в очках. Она поднимает глаза от книги.

— Следователь? — Она смотрит на ваше удостоверение. — Давненько к нам никто не заглядывал. # speaker:librarian

— Мне нужны документы по истории города. Довоенные. И раньше, если есть. # speaker:sorokin

— Раньше — мало что осталось. Война, пожар в пятьдесят первом... Но кое-что есть. Пойдёмте. # speaker:librarian

Она ведёт вас в подвал. Узкая лестница, запах плесени и старой бумаги. # style:atmosphere # intensity:medium
...
Подвал библиотеки — больше, чем ожидалось. Ряды металлических стеллажей. Картонные коробки, подписанные от руки. # style:atmosphere

— Вот здесь — всё, что осталось от довоенного периода. — Библиотекарша указывает на дальний угол. — Там — экспедиции. Геологи, исследователи. Много рапортов. # speaker:librarian

Она уходит наверх. # style:action

// ═══════════════════════════════════════════════════════════════════════════════
// РАЗБЛОКИРОВАНИЕ: hidden_archive
// ═══════════════════════════════════════════════════════════════════════════════

{ not (SecretLocations ? hidden_archive):
    ~ SecretLocations += hidden_archive
    
    # location
    Открыта локация: Скрытый архив библиотеки
}

Вы начинаете просматривать документы. # style:action

Первая коробка — бухгалтерия. Вторая — партсобрания. Скука. # style:thought

Но третья... # style:dramatic # intensity:high

"ЭКСПЕДИЦИЯ 1890 ГОДА. УРАЛЬСКОЕ ГЕОЛОГИЧЕСКОЕ ОБЩЕСТВО. СТРОГО КОНФИДЕНЦИАЛЬНО." # style:document

Вы открываете. # style:action

Пожелтевшие страницы. Рукописный текст, местами выцветший. # style:atmosphere

"14 августа 1890. Обнаружена система пещер под северным склоном. Происхождение — не карстовое. Стены покрыты символами неизвестного происхождения. Рабочие отказываются входить — говорят о 'голосах'." # style:document # intensity:medium

{ not (CultLore ? lore_expedition_1890):
    ~ CultLore += lore_expedition_1890
    ~ lore_depth += 10
}

Вы листаете дальше. # style:action

"20 августа. Нашли артефакт. Каменная табличка с символами. Отправлена в Петербург для изучения. Но копию я сохранил. Что-то в этих знаках... они как будто живые." # style:document # intensity:high

На отдельном листе — рисунок. Тот самый символ. Три линии в красном круге. # style:horror # intensity:high

-> ep2_library_artifacts

=== ep2_library_artifacts ===

# mood: horror

Вы продолжаете искать. # style:action

За стеллажом — старый сундук. Замок — сломан. # style:atmosphere

Внутри — тряпьё. Но под ним... # style:dramatic # intensity:high

* [Достать]
    Вы осторожно разгребаете ткань. # style:action
    
    Маска. Деревянная, почерневшая от времени. Лицо — не человеческое. Звериное? Или что-то древнее. Глаза — пустые прорези, но кажется, что они смотрят. # style:horror # intensity:high
    
    { not (AncientArtifacts ? artifact_shaman_mask):
        ~ AncientArtifacts += artifact_shaman_mask
        ~ cult_awareness += 3
        
        # artifact
        Найден артефакт: Шаманская маска (очень древняя)
    }
    
    Под маской — нож. Костяной. Лезвие — из обсидиана. Рукоять — резная кость, покрытая символами. # style:horror # intensity:high
    
    { not (AncientArtifacts ? artifact_bone_knife):
        ~ AncientArtifacts += artifact_bone_knife
        ~ cult_awareness += 3
        
        # artifact
        Найден артефакт: Костяной ритуальный нож
    }
    
    -> lose_sanity_safe(3) ->
    
    Вы чувствуете... холод. Исходящий от этих предметов. Древний. Голодный. # style:thought # intensity:high
    
    -> ep2_library_end

* [Оставить]
    Что-то подсказывает — лучше не трогать. # style:thought # intensity:medium
    
    -> ep2_library_end

=== ep2_library_end ===

Вы поднимаетесь наверх. # style:action

Библиотекарша смотрит на вас странно. # style:atmosphere # intensity:medium

— Нашли что искали? # speaker:librarian

— Возможно. # speaker:sorokin

Она кивает. Молча. Её глаза — старые, усталые — но в них что-то мелькает. Понимание? Страх? # style:thought # intensity:medium

— Будьте осторожны, товарищ следователь. История этого города... она не любит, когда её ворошат. # speaker:librarian

-> episode2_morning

// ═══════════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Конфликт интересов — рассказать Громову о Вере
// ═══════════════════════════════════════════════════════════════════════════════
=== ep2_tell_gromov_about_vera ===
...
Громов сидит за столом. Бутылка — та же, что вчера. Или новая? Трудно сказать. # style:atmosphere

— А, Сорокин. Что-то нашли? # speaker:gromov

Вы садитесь напротив. Смотрите ему в глаза. # style:action

— Доктор Холодова. Из больницы. Она ведёт исследования. Собирает данные о... странных случаях. # speaker:sorokin

Громов замирает. Его рука — с рюмкой — останавливается на полпути к губам. # style:atmosphere # intensity:medium

— Что за исследования? # speaker:gromov

* [Рассказать всё (Громов +10, Вера -30)]
    Вы рассказываете. О медкартах. О паттернах в мозге. О её подозрениях насчёт завода. # style:action
    
    Громов слушает. Его лицо — каменное. Но глаза... глаза выдают интерес. И что-то ещё. Расчёт? # style:thought # intensity:medium
    
    — Интересно. Очень интересно. — Он ставит рюмку. — Спасибо, Сорокин. Вы мне... помогли. # speaker:gromov
    
    ~ tell_npc_about("gromov", "vera")
    
    # conflict
    КОНФЛИКТ ИНТЕРЕСОВ: Вы рассказали Громову о Вере. Её доверие упало на 30. Она может узнать.
    
    -> episode2_morning

* [Намекнуть, но не раскрывать детали]
    — Она знает больше, чем показывает. Но... я бы не хотел, чтобы это стало известно. Пока. # speaker:sorokin
    
    Громов смотрит на вас. Оценивает. # style:thought # intensity:medium
    
    — Понимаю. Ваше расследование — ваши правила. — Он кивает. — Но помните: здесь все связаны. Все знают друг о друге. # speaker:gromov
    
    ~ trust_gromov += 5
    
    -> episode2_morning

* [Отступить — не рассказывать]
    — Извините, Степан Петрович. Я... погорячился. Это ничего важного. # speaker:sorokin
    
    Громов смотрит на вас долго. Потом — усмехается. # style:action
    
    — Врёте плохо, Сорокин. Но ладно. Ваше дело. # speaker:gromov
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Клава даёт водку для "подкупа" местных
// ═══════════════════════════════════════════════════════════════════════════════

Клава оглядывается. Достаёт из сумки бутылку "Столичной". # style:action

— Возьмите. — Она протягивает вам бутылку. — В этом городе... иногда язык развязывается только так. Люди боятся. Но после стакана — смелеют. # speaker:klava

{ not (inventory ? item_vodka):
    ~ inventory += item_vodka
    
    # item
    Получен предмет: бутылка водки (можно использовать для подкупа)
}

— Спасибо, Клавдия Петровна. # speaker:sorokin

— Берегите себя, товарищ следователь. # speaker:klava
...
// РАСШИРЕННАЯ СИСТЕМА РАССУДКА: Обновляем осведомлённость NPC
~ update_npc_sanity_awareness()

// Проверка: NPC может отказаться говорить при очень низком рассудке
{ npc_refuses_due_to_sanity("vera"):
    Больница №1. # style:title # intensity:medium
    
    Вы входите, но медсестра на посту — останавливает вас. # style:atmosphere # intensity:medium
    
    — Доктор Холодова сейчас занята. — Она смотрит на вас с нескрываемым страхом. — И... и она просила передать, что не сможет вас принять. # speaker:nurse
    
    Вы видите краем глаза — Вера за стеклянной дверью. Она смотрит на вас. И качает головой. # style:atmosphere # intensity:high
    
    Она боится. Боится ВАС. Того, что видит в ваших глазах. # style:thought # intensity:high
...
{ told_gromov_about_vera:
    Вера смотрит на вас. Её взгляд — холодный. Знающий. # style:atmosphere # intensity:high
    
    — Вы рассказали Громову. О моих исследованиях. — Это не вопрос. — Я видела, как он на меня смотрел. # speaker:vera # intensity:high
    
    — Вера Павловна, я... # speaker:sorokin
    
    — Не надо. — Она отворачивается. — Я понимаю. Вам нужны союзники. Но теперь... теперь мне придётся быть осторожнее. # speaker:vera # intensity:medium
    
    ~ trust_vera -= 20
    ~ vera_danger_level += 1
    
    Она не даст вам ключ. Не сейчас. Может быть — никогда. # style:thought # intensity:high
    
    -> episode2_morning
}

// Проверка: может ли Вера предать при низком доверии
{ will_betray("vera"):
    Вера смотрит на вас странно. В её глазах — что-то расчётливое. # style:atmosphere # intensity:medium
    
    — Вы знаете, товарищ Сорокин... — Она говорит медленно, тщательно подбирая слова. — Астахов спрашивал о вас. О вашем расследовании. # speaker:vera # intensity:medium
    
    Холодок пробегает по спине. # style:horror # intensity:medium
    
    — И что вы ему сказали? # speaker:sorokin
    
    Она не отвечает. Только улыбается. Тонко, безрадостно. # style:atmosphere # intensity:high
    
    ~ betrayed_by_vera = true
    ~ trust_vera -= 10
    
    Вы больше не можете доверять Вере. Она выбрала сторону. # style:thought # intensity:high
...
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Эксклюзивная улика от Веры
{ can_get_exclusive_clue_from("vera") && not has_exclusive_clue(excl_vera_medcards):
    — Тогда... вот. — Она достаёт из сейфа ещё одну папку. Толще первой. # speaker:vera # intensity:medium
    
    — Медицинские карты. Тех, кто работал в секретном отделе. Включая... — Она замолкает. — Включая тех, кого официально не существует. # speaker:vera # intensity:high
    
    ~ give_exclusive_clue(excl_vera_medcards)
    
    # clue
    ЭКСКЛЮЗИВНАЯ УЛИКА: Секретные медицинские карты проекта "Эхо"
    
    Карты показывают: эксперименты над людьми. Добровольцы... и не только. # style:important # intensity:high
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Вера даёт лекарство как врач
// ═══════════════════════════════════════════════════════════════════════════════

Вера идёт к шкафчику с медикаментами. Достаёт пузырёк. # style:action

— Возьмите. Седативное. Мой собственный рецепт. — Она смотрит на вас с беспокойством. — Судя по вашим глазам, вы уже слышали голоса. Это поможет... на время. # speaker:vera

{ not (inventory ? item_medicine):
    ~ inventory += item_medicine
    
    # item
    Получен предмет: успокоительное (восстанавливает рассудок)
}

— Спасибо. # speaker:sorokin

— Только не злоупотребляйте. И... будьте осторожны. # speaker:vera

// Квест Люды — Церебролизин
{ quest_lyuda_medicine && not quest_lyuda_medicine_done && not (inventory ? item_cerebrolizin):
    
    + [Вера Степановна, у меня ещё одна просьба...]
        — Слушаю. # speaker:vera
        
        — На рынке есть девушка, Люда. У её матери тяжёлое состояние — деменция. Ей нужен Церебролизин. Не могли бы вы помочь? # speaker:sorokin
        
        Вера снимает очки. Трёт переносицу.
        
        — Церебролизин... Редкое лекарство. В области — дефицит. # speaker:vera
        
        Пауза. Она смотрит на шкафчик.
        
        — Но у меня есть запас. Для особых случаев. — Она достаёт упаковку. — Возьмите. Передайте девушке. # speaker:vera
        
        — Сколько я должен? # speaker:sorokin
        
        — Ничего. — Вера качает головой. — Люда... я знаю её мать. Хорошая женщина была. До болезни. Если это поможет... # speaker:vera
        
        ~ inventory += item_cerebrolizin
        
        # item
        Получен предмет: Церебролизин (для матери Люды)
...
* [Кто "они"?]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // СВЯЗЬ С ПОБОЧНЫМ РАССЛЕДОВАНИЕМ: Николай Семёнович
    // ═══════════════════════════════════════════════════════════════════════════
    { trust_fyodor >= 30:
        — Но есть и те, кто... вышел. — Фёдор смотрит в окно. — Был один. Николай. Николай Семёнович. Работал на заводе охранником. Был в культе двадцать лет. # speaker:fyodor # intensity:medium
        
        — Что с ним стало? # speaker:sorokin
        
        — В восемьдесят третьем... они забрали его сына. Для ритуала. — Фёдор качает головой. — После этого Николай исчез. Одни говорят — убили. Другие — что он прячется. Ждёт. # speaker:fyodor # intensity:high
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Фёдор даёт отмычку и раскрывает свой первородный грех
// ═══════════════════════════════════════════════════════════════════════════════

Фёдор идёт к ящику в углу. Достаёт что-то. # style:action

— Вот. — Он протягивает вам набор отмычек. Старых, потёртых, но рабочих. — Здесь много замков. Тайников. Я двадцать лет собирал эти знания. # speaker:fyodor

{ not (inventory ? item_lockpick):
    ~ inventory += item_lockpick
    
    # item
    Получен предмет: набор отмычек (можно открывать тайники)
}

— Откуда они у вас? # speaker:sorokin

Фёдор молчит. Долго. Потом — говорит: # style:dramatic # intensity:high

— В пятьдесят третьем... я не просто нашёл пещеры. Я открыл Дверь. Первый раз. — Его голос — надломленный. — Не знал, что делаю. Любопытство. Молодость. Глупость. # speaker:fyodor # intensity:high

— Что случилось? # speaker:sorokin

— ОНО вышло. Ненадолго. Тень. Но этого хватило. Трое погибли в ту ночь. Мои товарищи. — Он закрывает глаза. — Моя вина. Мой грех. С тех пор — не могу уйти. Пока не закрою то, что открыл. # speaker:fyodor # intensity:high

{ not (CharacterSecrets ? fyodor_original_sin):
    ~ CharacterSecrets += fyodor_original_sin
    ~ understanding_fyodor += 25
}
...
— Изъяты. — Человек в штатском не моргает. — По распоряжению. # speaker:escort
...
// ═══════════════════════════════════════════════════════════════════════════════
// НОВАЯ ЛОКАЦИЯ: Квартира Зориных
// ═══════════════════════════════════════════════════════════════════════════════
=== ep2_zorin_apartment ===

~ actions_today = actions_today + 1
~ visited_new_locations += loc_zorin_apartment

# mood: emotional
# title: Квартира Зориных

Таня ведёт вас через дворы, мимо детской площадки с обледеневшими качелями, к серой пятиэтажке. # style:atmosphere # intensity:medium

— Здесь. — Она останавливается у подъезда. Её голос — еле слышный. — Я не была здесь с тех пор, как... # speaker:tanya

Она не заканчивает. # style:thought

Подъезд — тёмный, пахнет сыростью и кошками. Третий этаж. Дверь — обитая дерматином, с латунной цифрой "17". # style:atmosphere

Таня достаёт ключ. Её рука дрожит. # style:action

— Я... я не могу. — Она отдаёт ключ вам. — Войдите один. Я подожду здесь. # speaker:tanya

* [Взять ключ]
    Вы берёте ключ. Холодный металл в ладони. # style:action
    
    — Спасибо. — Вы смотрите ей в глаза. — Это важно. # speaker:sorokin
    
    Она кивает. Отворачивается к окну. # style:action
    
    -> ep2_zorin_apartment_inside
    
* [Предложить войти вместе]
    — Таня. — Вы кладёте руку ей на плечо. — Вместе. Это ваш дом. # speaker:sorokin
    
    Она смотрит на вас. В её глазах — благодарность. И страх. # style:atmosphere # intensity:medium
    
    — Ладно. — Голос дрожит. — Вместе. # speaker:tanya
    
    ~ trust_tanya += 5
    ~ gain_sanity(2)
    
    -> ep2_zorin_apartment_inside_together
...
Квартира — как капсула времени. Всё замерло в октябре прошлого года. # style:atmosphere # intensity:medium

Тапочки у входа — мужские, 43-й размер. Пальто на вешалке — тёмно-синее, с прожжённой дырой на рукаве. На полке — пустая пепельница, засохший цветок в горшке. # style:atmosphere

Пыль. Везде — пыль. # style:atmosphere # intensity:low

Гостиная — маленькая, тесная. Диван, телевизор, сервант с посудой. На стене — фотографии: Таня в школьной форме, свадебное фото, какой-то пикник у реки. # style:atmosphere

Кухня — узкая, с газовой плитой и облезлым холодильником. На столе — чашка с засохшим чаем. Газета "Правда" за 21 октября 1985 года. # style:atmosphere # intensity:medium

{ zorin_apt_searched:
    Вы уже были здесь. Ничего нового. # style:thought
    -> ep2_zorin_apartment_end
}

Куда смотреть? # style:thought
...
* [Тайник (только при высоком доверии Тани)]
    { can_get_exclusive_clue_from("tanya"):
        Таня говорила о тайнике. В детстве она видела, как отец прячет что-то за книгами. # style:thought
        -> ep2_zorin_secret
    - else:
        Таня упоминала что-то про тайник, но не сказала где. Нужно больше её доверия. # style:thought
        -> ep2_zorin_apartment_inside
    }
...
Вы входите вместе. Таня держится за вашу руку — крепко, как ребёнок. # style:atmosphere # intensity:medium
...
Кабинет — крошечная комната, три на три метра. Стол, заваленный бумагами. Стеллаж с книгами. Карта города на стене — с пометками красным карандашом. # style:atmosphere # intensity:medium

Вы изучаете стол. # style:action

Чертежи — какие-то вентиляционные шахты? Схемы трубопроводов? # style:thought # intensity:medium

{ not found_zorin_letters:
    Под стопкой чертежей — конверты. Четыре письма. Без обратного адреса. # style:important # intensity:high
    
    ~ found_zorin_letters = true
    
    // Добавляем в основную систему улик
    { not (CluesE ? zorin_warning_letters):
        ~ CluesE += zorin_warning_letters
        ~ sync_evidence_count()
    }
    
    # clue
    УЛИКА: Письма Зорину (анонимные предупреждения)
    
    Вы читаете. Почерк — незнакомый, аккуратный. # style:action
    
    "Алексей. Они знают о твоих поисках. Будь осторожен."
    
    "Встреча — 15 октября, полночь. Старая шахта. Принеси то, что нашёл."
    
    "Ты был прав насчёт А.Ч. Он — ключ ко всему. Но не доверяй Г."
    
    "Прощай, друг. Если читаешь это — меня уже нет. Дверь открылась."
    
    А.Ч. — Академик Чернов? Г. — Громов? Письма связывают Зорина с кем-то, кто знал правду. И погиб за неё. # style:thought # intensity:high
    
    ~ cult_awareness += 3
    ~ lore_depth += 5
}

Карта на стене. # style:thought

Красные точки — места исчезновений? Линии между ними образуют... символ? # style:thought # intensity:high
...
Спальня. Двуспальная кровать, аккуратно застеленная. Шкаф с одеждой. Тумбочка с будильником, остановившимся на 3:47. # style:atmosphere

{ not found_zorin_photos:
    В тумбочке — фотоальбом. # style:action
    
    ~ found_zorin_photos = true
    
    Вы листаете. Детские фото Тани. Семейные праздники. # style:atmosphere # intensity:low
    
    И — последние страницы. # style:dramatic # intensity:high
    
    Фотографии леса. Ночные. Красный свет между деревьями. Размытые фигуры. # style:horror # intensity:high
    
    На одной — чётко виден символ. Вырезанный на коре дерева. # style:important
    
    // Добавляем в основную систему улик
    { not (CluesE ? zorin_night_photos):
        ~ CluesE += zorin_night_photos
        ~ sync_evidence_count()
    }
    
    # clue
    УЛИКА: Ночные фотографии Зорина
    
    Зорин следил за культом. Документировал их ритуалы. Значит — он знал, на что идёт. И всё равно исчез. # style:thought # intensity:high
...
Тайник. За книжной полкой в кабинете — выдвижная панель. # style:atmosphere # intensity:high

Вы тянете. Панель поддаётся. # style:action

Внутри — жестяная коробка из-под печенья. # style:atmosphere

~ zorin_apt_searched = true

Вы открываете. # style:action # intensity:high

// ЭКСКЛЮЗИВНАЯ УЛИКА — только при высоком доверии Тани
// ВАЖНО: Это ОТДЕЛЬНАЯ улика от дневника Зорина (excl_tanya_diary)!
{ can_get_exclusive_clue_from("tanya") && not has_exclusive_clue(excl_chernov_letters):
    Письма. Десятки писем. # style:important # intensity:high
    
    От Чернова. Академика Чернова. # style:dramatic # intensity:high
    
    "Алексей, ты был прав. Проект "Эхо" — это не наука. Это жертвоприношение."
    
    "Дверь — реальна. Я видел её. Видел то, что за ней."
    
    "Они используют нас. Меня, тебя, всех. Мы — топливо для их ритуала."
    
    "Я нашёл способ закрыть её. Но цена... цена высока."
    
    И последнее письмо — датировано 20 октября: # style:important # intensity:high
    
    "Беги, Алексей. Беги и не оглядывайся. Забери Таню. Уезжай из этого города навсегда. Я попытаюсь... попытаюсь всё исправить. Прости меня."
    
    ~ give_exclusive_clue(excl_chernov_letters)
    
    # clue
    ЭКСКЛЮЗИВНАЯ УЛИКА: Письма Чернова к Зорину
    
    Связь между Черновым и Зориным подтверждена. Они оба знали правду о культе и проекте "Эхо". # style:important
...
* [Взять всё]
    Вы забираете коробку. # style:action
...
// Квартира посещается вечером ("После работы. В шесть.")
~ time_of_day = 2

Вы выходите. Таня ждёт на лестничной площадке. За окном — темнеет. # style:atmosphere

{ found_zorin_letters || found_zorin_photos:
    — Нашли что-нибудь? # speaker:tanya
    
    Вы киваете. Не говорите что именно. Не сейчас. # style:thought
    
    — Пойдёмте. — Вы берёте её под руку. — Здесь больше нечего искать. # speaker:sorokin
    
    ~ trust_tanya += 5
}

{ not found_zorin_letters && not found_zorin_photos:
    — Пусто? # speaker:tanya
    
    — Почти. — Вы не хотите её расстраивать. # speaker:sorokin
}

// Вечер — переход к вечерней части эпизода 2
-> episode2_evening

// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма — встреча у шахты
// ═══════════════════════════════════════════════════════════════════════════════
=== ep2_anonymous_meeting ===

# mood: horror

Старая шахта. Окраина города. # style:title # intensity:high

Солнце село полчаса назад. Темнота — густая, вязкая. Фонарик режет её узким лучом. # style:atmosphere # intensity:high

Шахта — заброшенная с пятидесятых. Деревянный копёр, покосившийся, гнилой. Ржавые рельсы, уходящие в темноту провала. # style:atmosphere

Тишина. Только ветер в деревьях. И ваше дыхание. # style:atmosphere # intensity:medium

-> lose_sanity_safe(3) ->

Вы ждёте. Минута. Пять. Десять. # style:action

{ sanity < 50:
    -> vision_false("Тень между деревьями. Человеческий силуэт. Неподвижный.") ->
}

— Вы пришли. # speaker:stranger # intensity:high

Голос — за спиной. Вы оборачиваетесь. # style:action

Человек. Немолодой — лет шестьдесят, может больше. Худой, сутулый. Лицо — в тени капюшона. # style:atmosphere # intensity:high

— Не бойтесь. — Он поднимает руки. — Я не враг. # speaker:stranger

— Кто вы? # speaker:sorokin

Долгая пауза. Он отступает на шаг. В лунном свете — его лицо. Измождённое. С глубокими шрамами. # style:atmosphere # intensity:high

— Меня звали... Николай Семёнович. Когда-то. Давно. До того, как... # speaker:stranger

— До того, как — что? # speaker:sorokin

— До того, как я стал частью этого. — Он указывает на лес. — До того, как они сделали меня... своим. # speaker:stranger # intensity:high

* [Вы — член культа?]
    — Культ. — Вы говорите ровно. — Вы из них. # speaker:sorokin
    
    Он вздрагивает. Как от удара. # style:action
    
    — Был. Был — один из них. Двадцать лет. До того, как... — Он трогает шрамы на лице. — До того, как я понял, что мы делаем. ЧТО мы кормим. # speaker:stranger # intensity:high
    
    -> ep2_anonymous_truth

* [Почему вы мне помогаете?]
    — Зачем вам это? — Вы делаете шаг вперёд. — Зачем рисковать? # speaker:sorokin
    
    — Искупление. — Его голос — еле слышный. — Я... я виноват. Во многом. В слишком многом. # speaker:stranger
    
    -> ep2_anonymous_truth

* [Это ловушка]
    — Достаточно. — Вы кладёте руку на пистолет. — Я ухожу. # speaker:sorokin
    
    — Подождите! — Он делает шаг вперёд. — Я... я могу показать. Доказать. # speaker:stranger
...
Он достаёт из-за пазухи свёрток. Протягивает вам. # style:action

— Здесь — всё. Имена. Даты. Места ритуалов. Схема подземелий. — Его руки дрожат. — Я собирал это годами. Ждал... кого-то, кому можно доверять. # speaker:stranger

Вы берёте свёрток. Разворачиваете. # style:action

Бумаги. Десятки страниц. Рукописные записи, схемы, фотографии. # style:important # intensity:high

— Они убили моего сына. — Его голос ломается. — Использовали для ритуала. Я... я держал его за руку, пока... — Он не заканчивает. # speaker:stranger # intensity:high

~ letter_author_known = true
~ active_sidequests += sq_letters_solved
~ active_sidequests -= sq_letters_trusted

# sidequest
ПОБОЧНОЕ РАССЛЕДОВАНИЕ ЗАВЕРШЕНО: Анонимные письма

Автор — Николай Семёнович, бывший член культа. Его сын был жертвой ритуала 1983 года.
...
{ not (CluesC ? insider_testimony):
    ...
    # clue
    УЛИКА: Свидетельство бывшего члена культа
}

~ cult_awareness += 8
~ lore_depth += 10

— Они узнают. — Он оглядывается. — Они всегда узнают. Но теперь... теперь у вас есть шанс. # speaker:stranger

— Что мне делать? # speaker:sorokin

— Найдите Чернова. Академика. Он — центр всего. Он создал... ЭТО. И только он знает, как закрыть Дверь. # speaker:stranger # intensity:high

Он отступает в тень. # style:action

— Прощайте, товарищ следователь. Мы больше не увидимся. # speaker:stranger

И исчезает. Растворяется в темноте между деревьями. # style:atmosphere # intensity:high

Вы стоите один. С пачкой документов в руках. С новым грузом на плечах. # style:thought

-> episode2_evening

// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма — выбор вечером
// ═══════════════════════════════════════════════════════════════════════════════
=== ep2_evening_letter_choice ===

Вы помните письмо. Шахта. Вечером. # style:thought # intensity:medium

* [Идти к шахте — рискнуть]
    -> ep2_anonymous_meeting
    
* [Не идти — слишком опасно]
    Нет. Это ловушка. Обязательно. # style:thought
    
    ~ active_sidequests -= sq_letters_trusted
    ~ active_sidequests += sq_letters_ignored
    
    -> ep2_evening_continue

=== ep2_evening_continue ===
// Продолжение вечера после отказа от встречи
-> episode2_evening
...
// ═══════════════════════════════════════════════════════════════════════════════
// ПОБОЧНОЕ РАССЛЕДОВАНИЕ: Анонимные письма — встреча с автором
// ═══════════════════════════════════════════════════════════════════════════════
{ (active_sidequests ? sq_letters_trusted) && letters_followed >= 1 && not letter_author_known:
    -> ep2_evening_letter_choice
}
...
// РАСШИРЕНИЕ: Рынок — можно узнать слухи перед заводом
* [На рынок — собрать информацию о заводе]
    Прежде чем идти на завод, стоит узнать, что говорят в городе. # style:thought
    -> location_market_day1to5
...
— Проходите. Но без сопровождения — никуда. # speaker:guard
...
В углу — небольшой сейф. Старый, советский. Замок — механический. # style:atmosphere

Но кое-что осталось.

// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Сейф который можно открыть отмычками
// ═══════════════════════════════════════════════════════════════════════════════
* { has_item(item_lockpick) } [Открыть сейф отмычками]
    Отмычки Фёдора снова пригождаются. Старый замок поддаётся через минуту. # style:action
    
    Внутри — папка. Толстая, в красной обложке с грифом "СОВЕРШЕННО СЕКРЕТНО". # style:important
    
    "ПРОЕКТ ЭХО — ФАЗА 4
    Субъект: Чернов А.М.
    Цель: Установление стабильного контакта с Сущностью
    Методика: Использование резонансных частот в сочетании с биологическим материалом (донорская кровь)
    
    РЕЗУЛЬТАТ (от 15.08.1983):
    Контакт установлен. Субъект утверждает о получении 'обещания'. Сущность требует жертв — регулярных, добровольных или нет.
    
    ПРЕДУПРЕЖДЕНИЕ: Субъект демонстрирует признаки полной лояльности Сущности. Рекомендовано немедленное отстранение.
    
    РЕЗОЛЮЦИЯ: Отклонено. Проект продолжается."
    ...
    # clue
    Улика найдена: секретные документы Фазы 4 — Чернов установил контакт с Сущностью!
    
    Это меняет всё. Чернов не просто учёный — он жрец. Посредник. # style:thought # intensity:high
...
"Моей Мариночке — навсегда. А.Ч. 1968"
...
{ trust_tanya >= 60:
    ...
    — Вы должны поесть. # speaker:tanya
...
Луна за окном — серп, растущий. Полнолуние — через двенадцать дней. 29 ноября. # style:important

{ knows_deadline:
    Но культ уже активен. Они готовятся. Собирают силы. # style:thought # intensity:medium
...
Полнолуние приближается. Культ готовится к ритуалу.

Но у вас есть время. Время узнать больше. Найти союзников. Подготовиться.
...
// Сообщение о приближении финала показывается в расширенном режиме
{ current_day >= 14:
    ПОЛНОЛУНИЕ СКОРО. Времени всё меньше.
...
// РАСШИРЕНИЕ: Рынок доступен в дневное время
* { time_of_day <= 1 } [На рынок — последние слухи перед финалом]
    -> location_market_day1to5
...
— Скоро ритуал. В полнолуние. Я планирую идти в пещеры — остановить их. # speaker:sorokin

Он долго молчит. # style:action

— Я знаю. — Серафим встаёт. — Вам понадобится защита. Встаньте на колени. # speaker:serafim
...
// ═══════════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Эксклюзивная улика от Фёдора
// ═══════════════════════════════════════════════════════════════════════════════
{ can_get_exclusive_clue_from("fyodor") && not has_exclusive_clue(excl_fyodor_safe_path):
    Фёдор замирает. Смотрит на вас — оценивающе. # style:atmosphere # intensity:medium
    
    — Вы... вы другой. Не как те, кто приезжал раньше. — Он идёт к тайнику под половицей. — Я хранил это двадцать лет. На крайний случай. # speaker:fyodor # intensity:medium
    
    Из-под половицы он достаёт свёрнутую карту. Старую, пожелтевшую, но детально прорисованную. # style:action
    
    — Полная схема подземелий. — Его голос дрожит. — Все коридоры. Все ловушки. Все выходы. И — путь мимо охраны к алтарю. # speaker:fyodor # intensity:high
    
    — Откуда это у вас? # speaker:sorokin
    
    — Я провёл там три месяца. В плену. Наблюдал. Запоминал. — Он смотрит вам в глаза. — И нашёл путь, который они не охраняют. Потому что считают его невозможным. # speaker:fyodor # intensity:high
    
    ~ give_exclusive_clue(excl_fyodor_safe_path)
    
    # clue
    ЭКСКЛЮЗИВНАЯ УЛИКА: Полная карта подземелий + безопасный путь
    
    С этой картой можно обойти все ловушки и охрану. # style:important # intensity:high
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИНТЕГРАЦИЯ УЛИК ИЗ КВАРТИРЫ: Письма Зорину упоминают "Г."
// ═══════════════════════════════════════════════════════════════════════════════
* { CluesE ? zorin_warning_letters } [Спросить про "Г." из писем]
    Вы достаёте конверты. Письма из квартиры Зорина. # style:action
    
    — "Не доверяй Г." — Вы читаете вслух. — Г. — это вы, Степан Петрович? # speaker:sorokin
    
    Громов смотрит на письма. Его лицо меняется — страх сменяется чем-то похожим на горечь. # style:atmosphere # intensity:high
    
    — Зорин... — Он берёт письмо. Руки дрожат. — Мы были друзьями. Когда-то давно. До всего этого. # speaker:gromov # intensity:high
    
    — Почему он вас предупреждал? # speaker:sorokin
    
    — Потому что я... — Громов закрывает глаза. — Я рассказал Астахову, что Алексей что-то нашёл. Я думал — это защитит мою семью. Я ошибся. # speaker:gromov # intensity:high
    
    ~ trust_gromov += 10
    ~ understanding_gromov += 15
    ~ lore_depth += 5
    
    Громов предал Зорина. И живёт с этим уже год. # style:thought # intensity:high
...
— Скоро полнолуние. Ритуал. В пещерах. # speaker:sorokin

— Я знаю. — Громов встаёт. — Когда придёт время... я пойду с вами. # speaker:gromov
...
// ═══════════════════════════════════════════════════════════════════════════════
// РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: Эксклюзивная улика от Громова
// ═══════════════════════════════════════════════════════════════════════════════
{ can_get_exclusive_clue_from("gromov") && not has_exclusive_clue(excl_gromov_key):
    Громов замирает. Смотрит на вас — долго, оценивающе. # style:atmosphere # intensity:medium
    
    — Подождите. — Он идёт к сейфу в углу. Набирает код. — Есть кое-что ещё. # speaker:gromov # intensity:medium
    
    Из сейфа он достаёт связку ключей и толстую папку. # style:action
    
    — Архив закрытых дел. — Его голос — хриплый. — Все исчезновения за последние сорок лет. Настоящие протоколы, не те, что в официальных отчётах. # speaker:gromov # intensity:high
    
    — Почему вы это хранили? # speaker:sorokin
    
    — Потому что надеялся. — Он смотрит в окно. — Надеялся, что однажды придёт кто-то... кто сможет это использовать. # speaker:gromov
    
    ~ give_exclusive_clue(excl_gromov_key)
    
    # clue
    ЭКСКЛЮЗИВНАЯ УЛИКА: Ключ от архива милиции + закрытые дела
    
    В папке — 47 случаев. Паттерн ясен: исчезновения происходят перед каждым полнолунием. # style:important # intensity:high
    
    ~ cult_awareness += 5
    ~ lore_depth += 10
}

// Проверка: выполнена ли просьба Громова (найти информацию о дочери)
{ (NPCRequests ? req_gromov_daughter) && not (completed_requests ? req_gromov_daughter) && (CluesE ? old_photos):
    Громов замечает папку в ваших руках. Его лицо меняется. # style:atmosphere # intensity:high
    
    — Это... это из старого архива? # speaker:gromov
    
    — Да. Я нашёл кое-что. — Вы достаёте фотографию. Девочка лет семи. Рыжие волосы. Улыбка. — Это Аня? # speaker:sorokin
    
    Громов берёт фотографию. Его руки дрожат. Губы беззвучно шевелятся. # style:atmosphere # intensity:high
    
    — Где... где вы это нашли? # speaker:gromov
    
    — В архиве библиотеки. Есть ещё записи. Она была... она была не одна. В ту ночь исчезли трое детей. Это был первый массовый ритуал культа. # speaker:sorokin # intensity:high
    
    Громов закрывает глаза. По щеке — слеза. # style:atmosphere # intensity:high
    
    — Спасибо. — Его голос — еле слышный. — Спасибо, что сказали правду. # speaker:gromov
    
    ~ complete_request(req_gromov_daughter)
    
    # quest
    ЗАДАНИЕ ВЫПОЛНЕНО: Информация о дочери Громова передана
...
— Мне нужна помощь. Скоро ритуал. В полнолуние. В пещерах. # speaker:sorokin

Громов бледнеет. # style:action

— Я... я не могу... # speaker:gromov

— Они похитили Серафима. И Веру. Зорин там. В полнолуние их убьют. # speaker:sorokin
...
{ CharacterSecrets ? chernov_wife:
    И ещё — фотография женщины. "Моей Мариночке — навсегда. А.Ч."
...
// ═══════════════════════════════════════════════════════════════════════════════
// ПЕРЕХОД К РАСШИРЕННОМУ РЕЖИМУ (день 5)
// ═══════════════════════════════════════════════════════════════════════════════
...
Рассвет. # style:atmosphere # intensity:high

// ═══════════════════════════════════════════════════════════════════════════════
// ПЕРЕХОД В РАСШИРЕННЫЙ РЕЖИМ (дни 5-15)
// Финал только на день 15 — полнолуние
// ═══════════════════════════════════════════════════════════════════════════════

Вы смотрите на город. На людей, спешащих по своим делам. На детей, бегущих в школу. # style:atmosphere

Нет. Рано. # style:thought

Вам нужно больше. Больше улик. Больше понимания. Больше союзников. # style:thought # intensity:medium

Есть ещё места, куда вы не заходили. Люди, с которыми не говорили. Тайны, которые город хранит. # style:thought

Вы изучаете лунный календарь. Полнолуние — через десять дней. 29 ноября. # style:important

Культ будет ждать. Они всегда ждали. Тридцать лет ждали — подождут ещё десять дней. # style:thought # intensity:medium

Десять дней. Достаточно, чтобы узнать всю правду. Чтобы найти всех союзников. Чтобы подготовиться. # style:dramatic

Вы возвращаетесь в город. Работа продолжается. # style:action
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Разблокирование cult_shrine — святилище культа
// ═══════════════════════════════════════════════════════════════════════════════
{ not (SecretLocations ? cult_shrine):
    ~ SecretLocations += cult_shrine

    # location
    Открыта локация: Святилище культа (финальное место силы)
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИНТЕГРАЦИЯ ИНТЕРЛЮДИИ: Связь с видением из кошмара
// ═══════════════════════════════════════════════════════════════════════════════
{ nightmares_won > 0:
    И вы замираете. # style:dramatic # intensity:high
    
    Это место. Вы ВИДЕЛИ его. В том кошмаре. Алтарь, фигуры в капюшонах, чаша... # style:thought # intensity:high
    
    Видение было правдой. Всё — было правдой. # style:important # intensity:high
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Артефакты в святилище — ритуальная мантия
// ═══════════════════════════════════════════════════════════════════════════════
У входа — на каменном выступе — лежит мантия. Чёрная, с красными символами. Древняя, истлевшая по краям. # style:atmosphere # intensity:high

{ not (AncientArtifacts ? artifact_ritual_robe):
    ~ AncientArtifacts += artifact_ritual_robe
    
    # artifact
    Найден артефакт: Ритуальная мантия культа
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Последний артефакт — карта первопроходцев
// ═══════════════════════════════════════════════════════════════════════════════
На краю алтаря — свиток. Пергамент, покрытый линиями и символами. Карта пещер — но древняя, нарисованная до всех экспедиций. # style:atmosphere

{ not (AncientArtifacts ? artifact_original_map):
    ~ AncientArtifacts += artifact_original_map
    ~ lore_depth += 5
    
    # artifact
    Найден артефакт: Оригинальная карта древнего культа
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ИСПРАВЛЕНИЕ: Раскрытие lore_chernov_rise — восхождение Чернова
// ═══════════════════════════════════════════════════════════════════════════════
...
— Я был молодым учёным тогда. Амбициозным. Приехал сюда в пятьдесят четвёртом, с первой группой исследователей. — Его голос меняется. Становится тише. — Тогда я ещё верил в науку. В прогресс. В будущее. # speaker:chernov

— Что изменилось? # speaker:sorokin

— ОНО. — Чернов указывает на Дверь. — Когда я впервые услышал голоса... увидел символы... я понял: наука — детская игрушка. Там, за Дверью — истинное знание. Истинная сила. # speaker:chernov # intensity:high

{ not (CultLore ? lore_chernov_rise):
    ~ CultLore += lore_chernov_rise
    ~ lore_depth += 15
    ~ cult_awareness += 5
}

— Я стал собирать единомышленников. Сначала — среди учёных. Потом — среди местных. Те, кто слышал голоса. Те, кто потерял близких и хотел вернуть. Те, кто искал... больше, чем может дать эта жизнь. # speaker:chernov

Он усмехается. Горько.

— Тридцать два года я строил это. Шаг за шагом. Жертва за жертвой. И сегодня — моя награда. # speaker:chernov
...
{ trust_astahov < -5:
    ...
    - else:
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        // РАСШИРЕННАЯ СИСТЕМА ДОВЕРИЯ: NPC может спасти игрока
        // ═══════════════════════════════════════════════════════════════════════════
        { will_save("tanya"):
            Но тут — крик. # style:dramatic # intensity:high
            
            — РУКИ ПРОЧЬ ОТ НЕГО! # speaker:tanya # intensity:high
            
            Таня бросается на Астахова. Бьёт его чем-то тяжёлым. Он падает. # style:action # intensity:high
            
            — Виктор! — Она помогает вам встать. — Вы в порядке? # speaker:tanya
            
            ~ saved_by_tanya = true
            ~ trust_tanya += 15
            ~ gain_sanity(5)
            
            Она спасла вам жизнь. # style:thought # intensity:high
        - else:
            { will_save("fyodor"):
                Но тут — тень. Движение. # style:dramatic # intensity:high
                
                Фёдор. Откуда он взялся? # style:thought
                
                Он сбивает Астахова с ног. Жёстко, эффективно — как человек, который знает, как драться. # style:action # intensity:high
                
                — Двигайся, следователь! — Фёдор протягивает руку. # speaker:fyodor
                
                ~ saved_by_fyodor = true
                ~ trust_fyodor += 15
                ~ gain_sanity(5)
                
                Он пришёл. Несмотря на страх. Несмотря на двадцать лет в тени. # style:thought # intensity:high
            - else:
                { will_save("serafim"):
                    Но тут — свет. Ослепительный, белый. # style:dramatic # intensity:high
                    
                    Серафим. С крестом в руке. # style:atmosphere # intensity:high
                    
                    — Изыди! — Его голос — громоподобный. — Изыди, порождение тьмы! # speaker:serafim # intensity:high
                    
                    Астахов отшатывается. Шипит. Бежит. # style:action
                    
                    ~ saved_by_serafim = true
                    ~ trust_serafim += 15
                    ~ gain_sanity(8)
                    
                    Вера и свет — против тьмы. Старик выбрал сторону. # style:thought # intensity:high
                - else:
                    -> lose_sanity_safe(10) ->
                    
                    Вы один. Никто не пришёл на помощь. # style:thought # intensity:high
                }
            }
        }
    }
